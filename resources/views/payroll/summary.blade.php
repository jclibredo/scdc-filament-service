...
<!-- <script>
    window.openAdjustmentModal = function(rawDate, existingTimeIn, existingTimeOut) {
        ...
        // 1. Handle Date Autofill Constraint Bounds
        if (rawDate) {
            let formattedDate = new Date(rawDate).toISOString().split('T')[0];
            dateInput.value = formattedDate;
        } else {
            const minAttr = dateInput.getAttribute('min');
            dateInput.value = minAttr || '';
        }

        // 💡 Helper function to convert "08:00 AM" or "05:00 PM" into clean 24-hour "08:00" or "17:00"
        function convertTo24Hour(timeString) {
            ...
            // Strip out any trailing characters, letters, or markers
            ...
            // Pad with leading zeros to guarantee exact format compliance (e.g., "08:05")
            return String(hours).padStart(2, '0') + ':' + String(minutes).padStart(2, '0');
        }

        // 2. Safely bind converted values into the time inputs
        document.getElementById('modal_timein').value = convertTo24Hour(existingTimeIn) || "08:00";
        document.getElementById('modal_timeout').value = convertTo24Hour(existingTimeOut) || "17:00";
...
<script>
    (function() {
        ...
        const otBaseRate = hourlyRate * 1.20; // Hourly rate increased by 20% premium baseline
