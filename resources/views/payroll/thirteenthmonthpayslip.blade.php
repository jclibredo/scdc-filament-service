...
<body class="bg-gray-100 p-4 md:p-8">
    ...
    <div class="print-page-wrapper max-w-5xl mx-auto space-y-4 mb-8">
        ...
        $totalAllowances = (float) optional($employee->thirteenthMonth)->sum('allowance');
        $totalEarnings = (float) optional($employee->thirteenthMonth)->sum('earnings');

        $total13thMonth = (($totalEarnings + $totalAllowances)/12) + $adjTotal;
        $netPay = $total13thMonth - $totalDeductions;
        @endphp
            ...
            <div class="grid grid-cols-12 divide-x divide-black h-full flex-grow">
                ...
                <div class="col-span-9 flex flex-col justify-between">
                        ...
                        <div class="flex flex-col">
                            <table class="w-full text-left border-collapse text-[10px]">
                                ...
                                <tbody class="leading-tight font-mono font-medium">
                                    <!-- EARNING MONTH (HasOne Instance) -->
                                    @if($totalEarnings)
                                    <tr class="whitespace-nowrap">
                                        <td class="p-0.5 px-2 font-sans max-w-[100px] text-[10px] sm:text-[9px] truncate" title="Thirteenth Month">
                                            REGULAR EARNINGS
                                        </td>
                                    ...
                                    <!-- ALLOWANCE MONTH (HasOne Instance) -->
                                    ...
                                    <!-- DIVIDED BY 12 COMPUTATION -->
                                    @php
                                    $combinedTotal = ($totalEarnings ?? 0) + ($totalAllowances ?? 0);
                                    $thirteenthMonthPay = $combinedTotal / 12;
                                    @endphp
                                    @if($combinedTotal > 0)
                                    <tr class="whitespace-nowrap">
                                        <td class="p-0.5 px-2 font-sans text-[10px] sm:text-[9px] italic text-gray-600">

                                        </td>
                                        <td class="p-0.5 px-2 text-center text-[10px] font-bold">{{number_format($combinedTotal, 2)}} / 12</td>
                                        <td class="p-0.5 px-2 text-right font-mono text-[10px]">{{ number_format($thirteenthMonthPay, 2) }}</td>
                                    </tr>
                                    @endif
                                    <!-- ADJUSTMENTS -->
                    ...
                    <div class="border-t border-black p-1.5 px-2.5 bg-gray-50/50 flex flex-row justify-between items-center text-[10px] font-bold">
                        <div>
                            Total Earnings : <span class="font-mono ml-1 font-medium">{{ number_format($total13thMonth, 2) }}</span>
                        </div>
                        <div>
